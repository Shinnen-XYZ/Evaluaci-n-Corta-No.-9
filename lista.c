...
Nodo *crearMemoria( void *dato, Nodo *ptr_sig, Nodo *ptr_ant ) {
...
void insertarAlFrente( Nodo **ini, Nodo **fin, void *inf ) {
    if( estaVacia( *ini ) == 1 ) {
        *ini = *fin = crearMemoria( inf, NULL, NULL );
    } else {
        Nodo *nuevo = crearMemoria( inf, *ini, NULL );
        (*ini)->ptr_ant = nuevo;
        *ini = nuevo;
    }
}
...
void insertarAlFinal( Nodo **ini, Nodo **fin, void *inf ) {
    if( estaVacia( *ini ) == 1 ) {
        *ini = *fin = crearMemoria( inf, NULL, NULL );
        ...
        Nodo *nuevo = crearMemoria( inf, NULL, *fin );
        (*fin)->ptr_sig = nuevo; 
        *fin = nuevo;
    }
}
